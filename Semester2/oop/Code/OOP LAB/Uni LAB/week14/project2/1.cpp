...
// File Names that will create to store and get data
...
const string authFile = "userAuthFile.txt";

// const file names because, file name will constant not chnage...


// Person base Class that is also abstract class 
class Person {
protected:
	string name;
	int age;
...
public:
	...
	virtual void display() = 0; // Now we must need to define this function in derived classes
	// pure virtual function, making Person an abstract class
	// it is also called polymorphism
};

// inheriting Person class to Patient class
...
public:
	...
	// Operator overloading: compare patients by ID
	...
	friend ostream& operator<<(ostream& os, const Patient& p) {
		os << "Patient ID: " << p.patientID << ", Name: " << p.name << ", Age: " << p.age
		   << ", Blood Group: " << p.bloodGroup << ", Admission Date: " << p.dateOfAdmission;
		return os;
	}
};


// inheriting Person class to Employee class
...
public:
	...
	void viewMyAppointments() {
		...
		ifstream appfile(appointmentFile); // ifstream used for read data from a file
		string line;
		bool found = false;
...
class AdminStaff: public Employee{
	...
	public:
	 ...
	 void manageStaff(){
		...
		fstream adminfile(adminStaffFile, ios::app); // app mode to add data to file without overwriting
...
class MedicalRecord{
	...
	public:
	 ...
	 void addEntry(){
		// For simplicity, we will just print the entry added
...
class Bill{
	...
	int amount;
	bool paymentStatus;
	string itemList[50]; // creating static array of items
	int itemCount; // static variable that because a object created it will increase by 1 to store data on next index
	int roomCharges;
	...
	public: 
	 ...
	 Bill(Patient p, Room r, int bid){
		// initializing member variables using constructor
		p1 = p;
		r1 = r;
		billID = bid;
		paymentStatus = false;
	 ...
	 void generateBill(){
		...
		for(int i = 0; i < itemCount; i++){
			cout << itemList[i];
			if(i != itemCount-1) // it will not print comma after last item
			 cout << ", "; 
		}
	...
	public:
	 ...
	 void addPatient(Patient *p){
		for(int i = 0;i<pCount;i++){
			if(*patients[i] == *p){ // use * to dereference pointer and compare objects, and also we dont use pointer etc
				cout << "Patient with ID " << p->getid() << " already exists!" << endl;
				return;
			}
		}
		if(pCount < 100){
	 ...
	 void viewAllDoctors() {
		 ...
		 if(!doctorfile.is_open()) {
			 // isOpen Function check that file is already created or not, to open the file
			 cout << "No doctor records found!" << endl;
			 return; 
			 // if file is not created then return from function
		 }
	 ...
	 void viewAllAppointments() {
		 ...
		 if(count == 1) { // means if while loop does not run then count will be 1...
	 ...
	 void viewAllEmployees() {
		 ...
		 while(getline(empfile, line)){
			 stringstream ss(line);
			 // use sstream to break the line into variables
	 ...
	 void viewHistory() {
		 // calling all view functions to show history
	 ...
	 void bookNewAppointment() {
		 ...
		 // Appointment will create based on the patient and doctor name provided
...
int Hospital::pCount = 0;
int Hospital::dCount = 0;
int Hospital::rCount = 0;
...
class UserAuthentication {
	...
	public:
		...
		void registerUser(string user, string pass, string role) {
			...
			// Registering user by saving data to auth file
			username = user;
			password = pass;
			userRole = role;

			// saving data in authfile
			fstream authfile(authFile, ios::app);
	...
	do {
		...
		switch (choice) {
			case 1: {
				...
				cin >> id;
				cin.ignore();
				// to remove the enter character key from memory buffer we used cin.ignore()
				cout << "Enter Name: ";
				getline(cin, name);
				cout << "Enter Age: ";
				cin >> age;
				...
				// Registering and saving doctor data that will add to hospital system
	...
	do {
		...
		switch (choice) {
			...
			case 3: {
				...
				// To add diagnosis we need doctor name and patient id
				Doctor tempDoctor(0, 0, "", "", "", "", auth.getUsername(), 0, "", "");
...
void operatorMenu(Hospital &hospital) { 
	// parameter hospital is passed by reference to avoid copying the entire object
	int choice;
	do {
		cout << "\n===== OPERATOR MENU =====" << endl;
		...
		switch (choice) {
			...
			case 3: {
				...
				// Bill will generated according to the patient and room details
				Patient tempPatient(0, "", "", pname, 0, "", ""); 
			...
			case 4: {
				...
				// bill payment processing based on patient and room details
				Bill bill(tempPatient, tempRoom, billID);
				bill.makePayment();
				break;
			}
...
void mainMenu() {
	...
	UserAuthentication auth;
	int mainChoice;
	fstream checkFile(authFile);
	if (!checkFile.is_open()) {
		cout << "\n===== HOSPITAL MANAGEMENT SYSTEM =====" << endl;
		cout << "=====================================" << endl;
		cout << "== Details == " << endl;
		// CREATING DEFAULT USERS, IF NO USER FILE FOUND
		...
		// also it will print on screen to show the default credentials
	...
	do {
		...
		if(cin.fail()) { // if user enter char instead of int then it will return true
		...
		switch (mainChoice) {
			...
			case 2: {
				...
				if (auth.login(username, password)) {
					// verifying login details for doctor
	...
	} while (mainChoice != 4); // if the choice will 4 then it will exit mean loop stop
} 

int main() {
	mainMenu();
	// calling mainMenu function that control all the project
	return 0;
}